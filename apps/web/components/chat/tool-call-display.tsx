        ...
        {isComplete && planOutput && (
          <details className="mt-1.5 sm:mt-2 text-xs">
            ...
            <div className="mt-1 max-h-32 sm:max-h-48 overflow-hidden">
              <CodeBlock className="bg-white border-border/60">
                <CodeBlockCode
  ...
  if (toolName === 'updatePlan') {
    ...
    return (
      <div className={cn(
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium">
          {isLoading && <Loader2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 animate-spin text-foreground/60" />}
          {isComplete && result?.success && <CheckCircle2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-emerald-600" />}
          {isError && <FileEdit className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-red-600" />}
        ...
        {content && mode === 'append' && (
          <div className="mt-1.5 sm:mt-2 rounded border border-emerald-200 bg-emerald-50/40">
            <div className="px-1.5 sm:px-2 py-1 text-[10px] sm:text-[11px] text-emerald-700 font-medium">Appended content</div>
              ...
              <pre className="text-[10px] sm:text-xs whitespace-pre-wrap text-foreground/80 overflow-x-auto">
                {content.split('\n').map((line, i) => (
                  <div key={i} className="flex items-start gap-1">
                    <Plus className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-emerald-600 mt-0.5 shrink-0" />
        ...
        {content && mode === 'replace' && (
          <div className="mt-1.5 sm:mt-2 rounded border border-blue-200 bg-blue-50/40">
            <div className="px-1.5 sm:px-2 py-1 text-[10px] sm:text-[11px] text-blue-700 font-medium truncate">
        ...
        {isComplete && result?.message && (
          <div className="mt-1.5 sm:mt-2 text-[10px] sm:text-xs text-green-600 font-medium">
            ✓ {result.message}
          </div>
        )}
      </div>
  ...
  return (
    <div className="my-1.5 sm:my-2 p-2 sm:p-3 rounded-lg border bg-muted/50 border-border">
      <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium">
        ...
        {isComplete && <CheckCircle2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-green-500" />}
      ...
      {Boolean(input) && Object.keys(input as Record<string, unknown>).length > 0 && (
        <details className="mt-1.5 sm:mt-2 text-[10px] sm:text-xs">
          ...
          <pre className="mt-1 p-1.5 sm:p-2 bg-white rounded overflow-x-auto text-[10px] sm:text-xs">
