...
export abstract class BaseAgent {
  ...
  private async getSandbox(): Promise<SandboxInstance> {
    ...
    if (this.config.sandboxId) {
      console.log("Running in agent base, resume sandbox", this.config.sandboxId);
  ...
  // Create a local checkpoint commit if there are uncommitted changes
  public async createLocalCheckpoint(
    ...
    // Stage and commit
    await sbx.commands.run(`cd ${this.WORKING_DIR} && git add -A`, {
      timeoutMs: 60000,
    });
    await sbx.commands.run(
      `cd ${this.WORKING_DIR} && git commit -m "${message.replace(/"/g, '\\"')}"`,
      { timeoutMs: 60000 }
    );

    // Return metadata and stats for the new HEAD
    return await this.getCommitMetadataAndStats();
  }

  // Retrieve commit metadata and machine-parse stats for a commit (defaults to HEAD)
  public async getCommitMetadataAndStats(sha?: string): Promise<{
    sha: string;
    message: string;
    stats: {
      filesChanged: number;
    ...
    const resolvedSha = (
      await sbx.commands.run(
        `cd ${this.WORKING_DIR} && git rev-parse ${sha ? sha : "HEAD"}`,
        { timeoutMs: 30000 }
      )
    ).stdout.trim();
    ...
    const message = (
      await sbx.commands.run(
        `cd ${this.WORKING_DIR} && git log -1 --pretty=%B ${resolvedSha}`,
        { timeoutMs: 30000 }
      )
    ).stdout.trim();

    // Minimal and reliable: single diff-tree call for per-file stats
    const numstatOutput = (
      await sbx.commands.run(
        `cd ${this.WORKING_DIR} && git --no-pager diff-tree --no-commit-id --numstat -r ${resolvedSha}`,
        { timeoutMs: 60000 }
      )
    ).stdout.trim();
    ...
    for (const line of numstatOutput.split("\n")) {
      ...
      const adds = addsStr === "-" ? 0 : Number(addsStr);
      const dels = delsStr === "-" ? 0 : Number(delsStr);
    ...
    return {
      sha: resolvedSha,
      message,
      stats: { filesChanged, additions, deletions, files },
    };
  }
  ...
  public async generateCode(
    ...
    try {
      ...
      // TODO: also refactor that agentSessionId is not used anywhere.
      // Always use working directory for all commands (repo is cloned directly into working directory)
      // If using Claude CLI with --dangerously-skip-permissions, ensure it runs as a non-root user
      let executeCommand = `cd ${this.WORKING_DIR} && ${commandConfig.command}`;
      // Run commands directly as the current user (image and sandbox are already set to 'agent')
